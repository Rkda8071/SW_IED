...
#define _DIST_MAX 500 // maximum distance to be measured (unit: mm)
...
//EMA
  dist_ema = dist_ema + (dist_raw - rec[cur])/n;
  rec[cur] = dist_raw;
  cur = (cur+1)%n;
...
if(dist_ema <= 180.0) {
     ...
     myservo.writeMicroseconds(_DUTY_MAX + (_DUTY_MIN - _DUTY_MAX)*(360.0 - dist_ema)/180.0);
  }
  else {
